// FILE: src/main/java/sistema/rotinas/primefaces/service/porteira/PorteiraBackupService.java
...
public class PorteiraBackupService implements IPorteiraBackupService {
    ...
    public PorteiraBackup obterPorPorteira(Long porteiraId) {
        return backupRepo.findByPorteira_Id(porteiraId).orElse(null);
    }

    @Override
    public List<PorteiraBackupUsuario> listarUsuariosDoBackup(Long backupId) {
        if (backupId == null) return Collections.emptyList();
    ...
    private PorteiraBackup executarBackupInterno(Long porteiraId, String origemExecucao, boolean notificarPorPorteira) {
        ...
        if (porteiraId == null) throw new IllegalArgumentException("porteiraId obrigatório.");

        PorteiraEletronica porteira = porteiraService.findById(porteiraId);
        if (porteira == null) throw new IllegalArgumentException("Porteira não encontrada: " + porteiraId);
        ...
        if (backup == null) {
            ...
            try {
                ...
                LOG.warn("[{}][BACKUP] Falha ao apagar usuários antigos backupId={} porteiraId={} msg={}",
                        origem, backup.getId(), pid, e.getMessage(), e);
            }
        ...
        LOG.info("[{}][BACKUP] Registro {} backupId={} (reutilizou={})",
                origem, (reutilizou ? "reutilizado" : "criado"), backup.getId(), reutilizou);
    ...
    public RestoreResult restaurarBackupParaPorteira(Long backupId, Long porteiraDestinoId, boolean dryRun,
        ...
        if (backupId == null) throw new IllegalArgumentException("backupId obrigatório.");
        if (porteiraDestinoId == null) throw new IllegalArgumentException("porteiraDestinoId obrigatório.");
        ...
        if (destino == null) throw new IllegalArgumentException("Porteira destino não encontrada: " + porteiraDestinoId);
