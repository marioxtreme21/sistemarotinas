...
@Service
public class RotinaPriceAutoSelectorService {
    ...
    /**
     * Regras:
     * - sempre considera apenas lojas com ArquivosPrice cadastrado
    ...
    public List<Long> selecionarLojasElegiveisHoje(boolean janelaRetry, String tagJanela) {
        ...
        LOG.info("[SCHED][PRICE][{}] Selecionando lojas elegíveis. janelaRetry={} data={}", tagJanela, janelaRetry, hoje);

        // 1) somente lojas com cadastro ArquivosPrice
        ...
        LOG.info("[SCHED][PRICE][{}] Lojas com cfg ArquivosPrice: {}", tagJanela, lojaIdsComCfg.size());

        if (lojaIdsComCfg.isEmpty()) {
            LOG.warn("[SCHED][PRICE][{}] Nenhuma loja com ArquivosPrice cadastrado.", tagJanela);
            return new ArrayList<>();
        }

        // 2) pega execuções do dia (somente PRICE + AUTO)
        List<RotinaExecucao> execAll = safeList(execRepo.findAll());

        List<RotinaExecucao> execucoesHoje = execAll.stream()
                .filter(e -> e != null
                        && e.getTipoRotina() == TipoRotinaEnum.PRICE
        ...
        LOG.info("[SCHED][PRICE][{}] Execuções AUTO hoje encontradas: {} (execIds={})",
                tagJanela, execIdsHoje.size(), (execIdsHoje.size() <= 20 ? execIdsHoje : "many"));

        // 3) se não tem execução hoje ainda:
        if (execIdsHoje.isEmpty()) {
            if (janelaRetry) {
                // ✅ ESTE É O COMPORTAMENTO QUE TE CONFUNDIU:
                // Retry só roda se já existiu execução hoje para ter falhas registradas.
                LOG.info("[SCHED][PRICE][{}] Janela de retry, mas NÃO existe execução AUTO hoje no banco. Nada a fazer.",
                        tagJanela);
                return new ArrayList<>();
            }

            LOG.info("[SCHED][PRICE][{}] Primeira janela do dia (sem histórico AUTO hoje). Elegíveis={}",
        ...
        LOG.info("[SCHED][PRICE][{}] Registros RotinaExecucaoLoja hoje (AUTO) encontrados: {}",
        ...
        for (Long lojaId : lojaIdsComCfg) {
            ...
            boolean teveSucessoHoje = hist.stream().anyMatch(x -> x != null && x.getStatus() == StatusExecucaoEnum.SUCESSO);
            if (teveSucessoHoje) {
            ...
            boolean teveFalhaOuParcialHoje = hist.stream().anyMatch(x ->
                    x != null && (x.getStatus() == StatusExecucaoEnum.FALHA || x.getStatus() == StatusExecucaoEnum.FALHA_PARCIAL));
        ...
        LOG.info("[SCHED][PRICE][{}] Seleção concluída. selecionadas={} bloqueadasPorSucesso={} retryElegiveis={} semHistoricoHoje={} totalComCfg={}",
    ...
    private Long safeLojaIdFromCfg(ArquivosPrice cfg) {
        try {
            if (cfg == null || cfg.getLoja() == null) return null;
            return cfg.getLoja().getLojaId();
        } catch (LazyInitializationException lie) {
            LOG.warn("[SCHED][PRICE] Lazy ao ler lojaId do cfg ArquivosPrice (ignorado). msg={}", lie.getMessage());
            return null;
        } catch (Exception e) {
            return null;
        }
    ...
    private Long safeExecucaoIdFromExecLoja(RotinaExecucaoLoja l) {
        try {
            ...
            // normalmente o ID do proxy pode ser lido sem inicializar tudo
            return l.getExecucao().getExecucaoId();
        } catch (LazyInitializationException lie) {
            LOG.warn("[SCHED][PRICE] Lazy ao ler execucaoId em RotinaExecucaoLoja (ignorado). msg={}", lie.getMessage());
            return null;
        } catch (Exception e) {
            return null;
        }
    ...
    private Long safeLojaIdFromExecLoja(RotinaExecucaoLoja l) {
        try {
            ...
            LOG.warn("[SCHED][PRICE] Lazy ao ler lojaId em RotinaExecucaoLoja (ignorado). msg={}", lie.getMessage());
            return null;
        } catch (Exception e) {
            return null;
        }
