...
@Named
@SessionScoped
public class RotinaPriceExecucaoBean implements Serializable {
    ...
    @Autowired
    private IRotinaPriceRunnerService runner;
    ...
    public void executar() {
        try {
            ...
            addMsg(FacesMessage.SEVERITY_INFO, "Rotina PRICE", "Execução registrada. ID: " + ultimaExecucaoId);
            ...
            addMsg(FacesMessage.SEVERITY_ERROR, "Rotina PRICE", "Falha ao executar: " + e.getMessage());
        }
    }
    ...
    public void carregarHistorico() {
        try {
            ...
            List<RotinaExecucao> price = all.stream()
                    .filter(e -> e != null && e.getTipoRotina() == TipoRotinaEnum.PRICE)
            ...
            this.historico = (price.size() > lim ? new ArrayList<>(price.subList(0, lim)) : new ArrayList<>(price));
    ...
    // ✅ compatibilidade com o xhtml: pesquisaPriceExecucao.xhtml chama recarregarHistorico
    public void recarregarHistorico() {
        carregarHistorico();
        addMsg(FacesMessage.SEVERITY_INFO, "Rotina PRICE", "Histórico recarregado.");
    }

    // ✅ compatibilidade com o xhtml: botão “Limpar seleção”
    public void limparSelecao() {
        try {
            ...
            addMsg(FacesMessage.SEVERITY_INFO, "Rotina PRICE", "Seleção limpa.");
        } catch (Exception e) {
            addMsg(FacesMessage.SEVERITY_WARN, "Rotina PRICE", "Não consegui limpar a seleção: " + e.getMessage());
        }
    }
            ...
            if (arquivoIds.isEmpty()) {
                ...
                etapasOrdenadas.sort(
                        Comparator
                                ...
                                .thenComparing(RotinaPriceExecucaoBean::safeInicioEtapa, Comparator.nullsLast(Comparator.naturalOrder()))
    ...
    public List<RotinaExecucaoArquivoEtapa> etapasPorArquivo(Long execucaoArquivoId) {
        ...
        return detalheEtapas.stream()
                ...
                .sorted(
                        Comparator
                                .comparing(RotinaPriceExecucaoBean::safeInicioEtapa, Comparator.nullsLast(Comparator.naturalOrder()))
                                .thenComparing(this::safeEtapaPk, Comparator.nullsLast(Comparator.naturalOrder()))
                )
                .toList();
    }
    ...
    private Long safeEtapaPk(RotinaExecucaoArquivoEtapa t) {
        ...
        try {
            // padrão do seu model atual
            if (t.getEtapaId() != null) return t.getEtapaId();
        } catch (Exception ignore) { }

        // fallback: tenta getters comuns por reflexão
